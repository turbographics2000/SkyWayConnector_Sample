...
export class Socket extends EventEmitter {
    ...
    updateCredential(newCredential) {
        ...
        const parseQuery = new URLSearchParams(this._io.io.opts.query);
        if (parseQuery.get('credential')) {
        ...
        this._io.io.opts.query = parseQuery.toString();
    ...
    _setupMessageHandlers() {
        config.MESSAGE_TYPES.SERVER.enums.forEach(type => {
            if (type.key === config.MESSAGE_TYPES.SERVER.OPEN.key) {
                this._io.on.call(this._io, type.key, openMessage => {
